...
//Rota que cria item de cardápio da categoria almoço.
router.post('/lunch', async (req, res) => {
  ...
  try {
    ...
    const item = await Lunch.findOne({ image });
    ...
    if (item) {
      ...
      const itemCreated = await Lunch.create(item);
...
//Rota que obtém um item de menu de almoço. 
router.get('/lunch', async (req, res) => {

  try {

    const itemList = await Lunch.find();
